...
- `EI` (`if (!cpu.imeEnableCountdown) cpu.imeEnableCountdown = 2` — decrement at the **end** of each instruction, including `EI`; IME flips when it hits 0). Full interrupt service is chapter 6; the countdown can exist now. Consecutive `EI`s do not reset an active countdown ([chapter 16](16-mooneye-polish.md)).
